    ...
    const loadData = async () => {
        try {
            ...
            try {
                ...
                // Profile not found, reset form but keep accountId to allow creating one
                setFormData({
                    ...initialFormState,
                    // Should we pre-fill something from the instagram account if available?
                    // Maybe brandName from username
                });
            }
        } catch (err: any) {
            if (err.response && err.response.status !== 404) {
            ...
            // If 404, it means no profile exists yet, which is fine, we reset form
            setFormData(initialFormState);
        }
    } catch (error) {
